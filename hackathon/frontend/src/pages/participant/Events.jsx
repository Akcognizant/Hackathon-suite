          ...
          {filtered.map((e) => (
            <div key={e.id} className="flex flex-col rounded-2xl border border-gray-100 bg-white p-6 shadow-sm">
              ...
              <dl className="mb-4 space-y-1 text-xs text-slate-500">
                <div className="flex justify-between"><dt>Dates</dt><dd className="font-medium text-slate-700">{e.startDate} → {e.endDate}</dd></div>
              ...
              <Button
                size="sm"
                className="w-full"
                disabled={(e.status || '').toUpperCase() === 'COMPLETED'}
                onClick={() => navigate('/portal/teams', { state: { hackathonId: e.id } })}
              >
                {(e.status || '').toUpperCase() === 'COMPLETED' ? 'Closed' : 'Form / join a team'}
              </Button>
            </div>
          ))}
        </div>
